...
1. http1.0 版本推出 Expires 字段表示过期时间（绝对时间），告诉浏览器在未过期之前就不用再请求资源了

    - 缺点：客户端与服务端时间不一致（时区不同导致），导致缓存失效

2. http1.1 版本推出 Cache-Control 字段，通过 max-age 字段表示过期时间（相对时间）

    - max-age 是一个相对时间，客户端与服务端就算时区不同，也不会受影响

    - Cache-Control 优先级高于 Expires

**弱缓存**

1. 服务端返回 304 代表命中弱缓存，直接从缓存中取；否则返回 200 并响应数据

2. Last-Modefied（http1.1 版本）：第一次请求时，服务端会告知客户端该资源上次更新的时间；再次请求时，客户端会携带这个更新时间到服务端，服务端会根据该资源的更新时间去判断资源是否有更新

    - 缺点：如果文件是通过服务器动态生成的，那么更新时间永远是最新生成的时间，尽管文件可能没有变化

3. etag（http1.1 版本）：存储的是文件的特殊标识；第一次请求时，服务端会携带该标识到客户端；再次请求时，客户端也会携带该标识到服务端
