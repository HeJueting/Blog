...
面试官，你好，我叫何爵廷。目前在中移物联网从事前端开发的相关工作，主要是负责 View 数据可视化这个产品的迭代和研发，在我简历中也有所体现，您也可以在线体验和使用。在大学期间就接触了前端，并且就加入学校工作室自学前端，大三期间也入职了京东开始前端实习的工作。毕业后，由于种种原因选择留在了重庆，目前基于个人发展和公司的变化，便决定离开重庆去互联网氛围较好的公司发展
...
3. 主线程和 worker 线程通过 postmessage 去通信，通过 onmessage 事件响应通信结果
...
1. 使用了 next.js，它增量再生的功能很强大（自动更新静态的 HTML 文件）

2. 首先在 百度 和 goole 上提交自己的网站，并提交 sitemap

3. 尝试用网上的工具自动生成 sitemap

    - 通过 react-router 使用 Link 的方式进行导航，最后打包出来的内容不是一个 a 标签的超链接，导致工具无法解析

    - 文章的超链接是动态生成的，并不是固定

4. 于是封装了一个脚本，每天定时更新我的 sitemap

5. 设置 title 和 meta 标签（description/keywords）
...
1. 将 CSS 配置信息保存在 mongodb 中

2. 使用 CSS 变量的功能实现

3. 换肤时，动态设置 style 标签中的 CSS 变量内容

</br>
</br>

### 集成 minio 对象存储服务 / 图片压缩访问 / 权限

1. 文件存储、文件存储、块存储

    - 块存储读写快，不利于共享，文件存储读写慢，利于共享

2. 提供一个接口去访问 minio 上的图片资源，使用 canvas 根据传入的宽高去绘制图片大小（原本传入的图片一般是 1280\*760）

3. 在 mongodb 中的数据与 minio 存储桶的数据进行关联，以此来扩展存储桶的权限功能
...
1. 字体大小利用 rem 控制，元素宽高利用 vw 去设置

2. 博客管理后台使用了 antd UI 库，自己写了一个脚本去将 px 转换为 rem
...
1. 博客系统跟管理后台通过监听 window 的 error 和 unhandledrejection 事件来进行事件捕获

    - error：捕获 JavaScript 运行时错误
    - unhandledrejection：捕获未 reject 的 promise

2. egg 自带了错误日志模块

3. 将错误日志以 txt 文件的形式保存在文件夹中，前端通过管理控制台去在线查看日志
...
1. 一开始固定每行展示的数量以及图片展示的高度，然后再利用 flex 来处理图片的等比缩放，让他们占满一行

2. 后来去调研了一下百度和 goole 的图片展示，发现了一种算法，可以很完美地展示图片：规定一个合适的图片高度，当一行只展示一张图片时算出它高度，如果不满足高度要求继续计算当一行展示两张图片时计算出图片的高度.....以此类推，直到计算出合适的图片高度

3. 最后一行特殊处理，规定他的高度为 200px
...
1. POP3/IMAP/SMTP 邮件协议

    - POP3：主要作用是接收邮件，当我们发送一封邮件后，POP3 服务器会接收到这封邮件并保存在服务器上，如果有客户端连接上了 POP3 服务器，它可以下载这封邮件，但是如果一旦下载成功，服务器上的这封邮件将会被删除

    - IMAP：也是用来接收邮件。不同点在于，它是双向通信的一个协议，服务器的邮件的状态跟客户端的邮件保持一致（删除、标记已读），这意味着客户端接收下载邮件后，IMAP 服务器仍然会保存这封邮件，不会立即删除

    - SMTP：用来发送邮件，如果收件人和发送人都是 qq.com 则直接转到 qq.com 的 POP3 服务器即可，如果收件人是 gmail.com，找到属于 gmail 的 SMTP 服务器，再由 gmail 的 SMTP 服务器转给 gmail 的 POP3 服务器

2. 需要去邮箱中开启 SMTP 的服务，并生成授权码，QQ 和网易都实践过，比较简单；goole 邮箱对机器人发邮件有所限制
...
1. 提供了两种模式，一种是静态生成，一种是服务端动态生成

    - 静态生成：打包的时候根据路由，自动构建静态的 HTML 文件；通过配置一个参数，可以实现静态增量再生

    - 服务端动态生成：每次访问该资源，都需要由 next 重新获取数据并生成一个最新的 HTML

2. 提供了一个 next/dynamic 可浏览器中动态加载模块，还可以设置 SSR 配置为 false，表示该模块不会在服务端运行，仅在浏览器端运行，这就意味着你可以在该模块中访问 window 和 dom 元素

3. 如果要使用 window 对象，需要借助 useEffect 这个 hook，传一个空数组模拟 componentDidMouted 这个 class 的生命周期
...
1. 目前所在的公司是一家国企，在员工晋升方面处理得不是很好

    - 公司名义上是每半年有一次晋升机会，但是公司将近 2/3 的人几年都没有再晋升过，脉脉上可看

    - 虽然个人实力存在一定的因素，但是公司属于移动体系，存在大量的移动体系关系户，他们会占用很多晋升的名额

2. 公司薪酬体系也处理得不好

    - 公司目前的 base 是基于 8 年前，也就是成立的时候的一个薪酬，加持没有普调，所以你一进入公司基本就是巅峰状态，80%甚至 90%的同事都不会再有调薪的机会，当然如果你职级升高了，可能会有调薪，但是据我所知，一般就 500-1500

    - 公司名义上每年有调薪的机会，也有跟领导聊过调薪的事情，得到回复是他没有权限去操作，也没有权限去跟领导申请

3. 目前公司的规划以及团队的规划

    - 自己想把 to C 的这一款可视化产品做好，但是现在领导已经把方向转到私有化部署，面向 to B 市场，现网的产品迭代缓慢，甚至之前提出的一些规划和需求，也在不断后移，因为 to B 市场能立马赚钱，to C 市场可能需要一段时期的铺垫以及推广

    - 目前的工作感觉无法体现自己的工作价值，也无法将自己的优势发挥至最大处

4. 技术氛围差

    - 现在团队产研一体化，售前，产品，前端，后端大家都在一个团队，但是各自负责的内容又完全不相同，国企的技术使用方面比较落后，也很少有技术沟通或者技术分享，很多人都是处于一种养老的状态，不利于我的技术发展
...
1. 团队的业务是一个纯粹的 C 端产品，这是跟我预期符合的（目前也在面试的其他厂，可能做的内容是偏向于内部系统）

2. 美团是一个互联网大厂，技术沉淀和技术要求都很适合我，我能给美团带来我的工作价值的同时，美团也提供给我一个巨大平台去技术深入和技术沉淀

3. 美团这几年的发展大家有目共睹，在外卖领域，饿了么至少在我生活的片区没有什么市场；外出觅食也会基于大众点评去参考；在重庆，滴滴是没有什么市场的，现在滴滴的业务已经不能与前几年相比，他的市场份额越来越小了，因此这块蛋糕我认为美团是有机会搏出来的，至少当知道上海还是哪个城市首推美团打车的时候，作为用户的我们是期待的
...
像美团这样的大厂，应该都是根据对应的职级去匹配对应的薪酬，根据前几轮的面试，公司应该也会给我一个合理的薪酬，而且作为标准化系统化的一家私企，薪酬体系和薪酬制度这块，我相信比我们物联网是要成熟的，不会存在很多不合理的情况，不然美团也没办法吸引到众多优秀人才了
...
1. 加入了外联部，为全校同学提供了 100 多个兼职的岗位

2. 参加了许多兼职，月入 1000 - 2000 不等

3. 由于酒精过敏，大二暑期决定开始搞技术，作为外来人员破格加入了经管学院的一个工作室，自学了半年的前端后，通过了京东的实习面试，最后也取得了转正的名额
...
2. 30 岁左右，因为自己产品思维比较活跃，也善于与人沟通，会尝试转型带技术团队
